 ...
 */
inline fun <T> Sequence(crossinline iterator: () -> Iterator<T>): Sequence<T> = object : Sequence<T> {
 ...
 * @sample samples.collections.Sequences.Building.sequenceFromIterator
 */
fun <T> Iterator<T>.asSequence(): Sequence<T> = Sequence { this }.constrainOnce()
...
fun <T> sequenceOf(vararg elements: T): Sequence<T> =
    if (elements.isEmpty()) emptySequence() else elements.asSequence()

/**
 * Returns an empty sequence.
 */
fun <T> emptySequence(): Sequence<T> = EmptySequence
...
private object EmptySequence : Sequence<Nothing>, DropTakeSequence<Nothing> {
    ...
    override fun drop(n: Long) = EmptySequence
    override fun take(n: Long) = EmptySequence
}
 ...
 */
@SinceKotlin("1.3")
fun <T> Sequence<T>.ifEmpty(defaultValue: () -> Sequence<T>): Sequence<T> = sequence {
 ...
 */
fun <T> Sequence<Sequence<T>>.flatten(): Sequence<T> = flatten { it.iterator() }
...
fun <T> Sequence<Iterable<T>>.flatten(): Sequence<T> = flatten { it.iterator() }
...
internal class FilteringSequence<T>(
    ...
    override fun iterator(): Iterator<T> = object : Iterator<T> {
        ...
        var nextState: Long = -1 // -1 for unknown, 0 for done, 1 for continue
        var nextItem: T? = null
...
internal inline fun checkIndexOverflow(index: Long): Long {
    if (index < 0) {
...
class TransformingIndexedSequence<T, R> constructor(
    private val sequence: Sequence<T>,
    private val transformer: (Long, T) -> R
) : Sequence<R> {
    override fun iterator(): Iterator<R> = object : Iterator<R> {
        val iterator = sequence.iterator()
        var index = 0
        override fun next(): R {
            return transformer(checkIndexOverflow(index + 1L), iterator.next())
        }
 ...
 */
data class IndexedValue<out T>(val index: Long, val value: T)
...
constructor(private val sequence: Sequence<T>) : Sequence<IndexedValue<T>> {
    override fun iterator(): Iterator<IndexedValue<T>> = object : Iterator<IndexedValue<T>> {
        ...
        override fun next(): IndexedValue<T> {
            return IndexedValue(checkIndexOverflow(index + 1L), iterator.next())
        }
...
internal interface DropTakeSequence<T> : Sequence<T> {
    fun drop(n: Long): Sequence<T>
    fun take(n: Long): Sequence<T>
}
...
internal class SubSequence<T>(
    ...
    private val count: Long get() = endIndex - startIndex

    override fun drop(n: Long): Sequence<T> =
        if (n >= count) emptySequence() else SubSequence(sequence, startIndex + n, endIndex)

    override fun take(n: Long): Sequence<T> =
        if (n >= count) this else SubSequence(sequence, startIndex, startIndex + n)
...
internal class TakeSequence<T>(
    ...
    override fun drop(n: Long): Sequence<T> = if (n >= count) emptySequence() else SubSequence(sequence, n, count)
    override fun take(n: Long): Sequence<T> = if (n >= count) this else TakeSequence(sequence, n)
...
internal class DropSequence<T>(
    ...
    override fun drop(n: Long): Sequence<T> =
        (count + n).let { n1 -> if (n1 < 0) DropSequence(this, n) else DropSequence(sequence, n1) }
...
constructor(
    ...
    override fun iterator(): Iterator<T> = object : Iterator<T> {
        val iterator = sequence.iterator()
        var dropState: Long = -1 // -1 for not dropping, 1 for nextItem, 0 for normal iteration
        var nextItem: T? = null
        ...
        override fun hasNext(): Boolean {
            if (dropState == -1L)
                drop()
            return dropState == 1L || iterator.hasNext()
        }
    }
}

internal class DistinctSequence<T, K>(private val source: Sequence<T>, private val keySelector: (T) -> K) :
    Sequence<T> {
    override fun iterator(): Iterator<T> = DistinctIterator(source.iterator(), keySelector)
}

private class DistinctIterator<T, K>(private val source: Iterator<T>, private val keySelector: (T) -> K) :
    AbstractIterator<T>() {
...
private class GeneratorSequence<T : Any>(
    ...
    override fun iterator(): Iterator<T> = object : Iterator<T> {
        var nextItem: T? = null
        var nextState: Long = -2 // -2 for initial unknown, -1 for next unknown, 0 for done, 1 for continue

        private fun calcNext() {
            nextItem = if (nextState == -2L) getInitialValue() else getNextValue(nextItem!!)
 ...
 *
 */
internal class ConstrainedOnceSequence<T> constructor(sequence: Sequence<T>) : Sequence<T> {
