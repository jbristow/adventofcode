...
data class Point2dLRange(
    ...
    constructor(a: Point2dL, b: Point2dL) :
        this(
    ...
    operator fun contains(point: Point2dL): Boolean = point.x in xRange && point.y in yRange
}
...
data class Point2d(
    ...
    override val neighbors: Set<Point2d>
        get() = (-1..1).flatMap { dx -> (-1..1).map { dy -> Point2d(x + dx, y + dy) } }.toSet() - this
...
data class Point3d(
    ...
    fun distance(other: Point3d): Double {
        val px = abs(x - other.x).toDouble()
        val py = abs(y - other.y).toDouble()
        val pz = abs(z - other.z).toDouble()
...
data class Point3dL(
    ...
    operator fun get(i: Int): Long =
        when (i) {
            ...
            1 -> y
            2 -> z
            else -> error("Illegal access, index $i")
        }

    @Suppress("DuplicatedCode")
    fun distance(other: Point3dL): Double {
        val px = abs(x - other.x).toDouble()
        val py = abs(y - other.y).toDouble()
        val pz = abs(z - other.z).toDouble()
        return sqrt(px * px + py * py + pz * pz)
...
data class Point3dBigDecimal(
    ...
    fun manhattanDistance(): BigDecimal =
        this.manhattanDistance(
            Point3dBigDecimal(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO),
        )

    override operator fun times(value: BigDecimal): Point3dBigDecimal =
        Point3dBigDecimal(
            x * value,
            y * value,
            z * value,
