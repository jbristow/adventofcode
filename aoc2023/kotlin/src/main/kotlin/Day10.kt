...
object Day10 : AdventOfCode() {
    ...
    private fun part1(input: List<String>): Long {
        ...
        val start = map.filterValues { it == Start }.keys.first()
    ...
    private fun Point2d.inside(boundary: Map<Point2d, PipeTile>): Boolean {
        ...
        val hCount = freq[Horizontal] ?: 0
        val westWalls = abs((freq[BendSW] ?: 0) - (freq[BendNW] ?: 0))
        val eastWalls = abs((freq[BendSE] ?: 0) - (freq[BendNE] ?: 0))

        return (hCount + westWalls) % 2 == 1 && (hCount + eastWalls) % 2 == 1
    }

    private fun determineStartTileType(map: Map<Point2d, PipeTile>, start: Point2d): PipeTile {
