import Day01.part1
import Day01.part2
import java.nio.file.Files
import java.nio.file.Paths
import kotlin.math.max

object Day01 {
    private const val FILENAME = "src/main/resources/day01.txt"

    fun fuelNeeded(mass: Int) = max((mass / 3) - 2, 0)

    tailrec fun totalFuelNeeded(mass: Int, fuel: Int? = null, totalFuel: Int = 0): Int {

        val nextFuel = when (fuel) {
            null -> fuelNeeded(mass)
            else -> fuelNeeded(fuel)
        }
        return when (nextFuel) {
            0 -> totalFuel
            else -> totalFuelNeeded(mass, nextFuel, totalFuel + nextFuel)
        }
    }

    fun part1() = Files.readAllLines(Paths.get(FILENAME))
        .sumBy { fuelNeeded(it.toInt()) }
        .toString()

    fun part2() = Files.readAllLines(Paths.get(FILENAME))
        .sumBy { totalFuelNeeded(it.toInt()) }
        .toString()
}

fun main() {
    println("Part 1: ${part1()}")
    println("Part 2: ${part2()}")
}
